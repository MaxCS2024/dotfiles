#!/usr/bin/env bash
#
# Install relay into ~/.local. Links by default, so editing a module in the
# repo takes effect immediately with no reinstall step.
#
# relay needs rig. If rig is not installed this stops rather than half-working:
# every module here logs, locks or notifies through it.
#
...
if [[ -z ${RIG_EXE:-} || ! -f ${RIG_EXE:-} ]]; then
    cat >&2 <<'MSG'
install: relay needs rig, and I cannot find it.
...
verify() {
    ...
    "$exe" bar hidden >/dev/null
    rig::log::success "relay $("$exe" version | head -1 | awk "{print \$2}") is installed, on rig $RIG_VERSION"
}

# ----------------------------------------------------------------------- go

if ((UNINSTALL)); then
    rig::log::info "removing relay from ${PREFIX/#$HOME/\~}"
    remove "$BIN_DIR/relay"
    remove "$LIB_DIR"
    ...
    if [[ -f $COMP_DIR/relay ]] &&
        [[ $(head -n1 -- "$COMP_DIR/relay") == "# generated by: relay completion bash" ]]; then
        run rm -f -- "$COMP_DIR/relay"
        printf '  removed %s\n' "${COMP_DIR/#$HOME/\~}/relay"
    else
        remove "$COMP_DIR/relay"
    fi
    # Copy-mode installs are not symlinks, so say so rather than guessing.
    [[ -e $BIN_DIR/relay || -e $LIB_DIR ]] &&
...
for f in "$HERE/relay" "$HERE/install.sh" "$HERE/tests/run"; do
    [[ -f $f && ! -x $f ]] && run chmod +x -- "$f"
done

place "$HERE/relay" "$BIN_DIR/relay" || exit 1
...
if ((COMPLETION)); then
    ...
    if ((DRY)); then
        ...
        mkdir -p -- "$COMP_DIR"
        RELAY_LIB_DIR="$HERE/lib" "$HERE/relay" completion bash >"$COMP_DIR/relay" &&
            printf '  %s (generated)\n' "${COMP_DIR/#$HOME/\~}/relay"
    fi
fi
