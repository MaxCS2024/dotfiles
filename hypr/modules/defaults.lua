...
--
-- Until the Conf menu reads `relay default list`, its Setup › Defaults
-- table (menu/ConfMenu.qml's defaultRoles) still marks the row in force by
-- comparing whole command strings with the ones built from this table, so
-- a candidate's bin, flatpak ref and args are spelled the same in both.
--
